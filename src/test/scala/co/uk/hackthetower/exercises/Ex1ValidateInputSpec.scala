...
class Ex1ValidateInputSpec extends FlatSpec with Matchers {
  ...
  it should "return Welcome for a valid Welcome entry with order on keys changed" in {
    val input = "Welcome(name=John_Is,round=66,maxslaves=-3,apocalypse=999)"
    Ex1ValidateInput.parseInput(input).getOrElse(Goodbye(Int.MinValue)) should be(Welcome("John_Is", 999, 66, -3))
  }
  ...
  it should "return React for a valid React entry" in {
    ...
    Ex1ValidateInput.parseInput(input).getOrElse(Goodbye(Int.MinValue)) should be(React(-77, "HAHA", 55, "ME", "IX", Some((9, 10)), Some((2, 3)), 5, Map("custom" -> "2", "custom2" -> "HEHE")))
  }

  it should "return React for a valid React entry in different order" in {
    val input = "React(generation=-77,name=HAHA,time=55,master=9:10,collision=2:3,slaves=5,custom=2,custom2=HEHE,view=ME,energy=IX)"
    Ex1ValidateInput.parseInput(input).getOrElse(Goodbye(Int.MinValue)) should be(React(-77, "HAHA", 55, "ME", "IX", Some((9, 10)), Some((2, 3)), 5, Map("custom" -> "2", "custom2" -> "HEHE")))
  }
